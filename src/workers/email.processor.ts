...
export class EmailProcessor extends WorkerHost {
  ...
  async process(job: Job<NotificationJobData>): Promise<void> {
    ...
    try {
      ...
      await this.emailChannel.send({ recipient, content, metadata });
      ...
      this.logger.log(`Email sent to ${recipient} [logId: ${logId}]`);
    } catch (error) {
      ...
      this.logger.error(
        `Failed to send email to ${recipient} [logId: ${logId}]: ${message}`,
      );
      throw error;
    }
  }
